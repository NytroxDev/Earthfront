import pygame
import pygame_gui
from core.game.settings import SettingsPanel
...
class MainMenu:
    ...
    def render(self):
        ...
        # Titre
        title_text = self.title_font.render("CARTE EXPLORER", True, self.title_color)
        title_rect = title_text.get_rect(center=(self.width // 2, self.height // 4 - 20))

        # Ombre du titre
        shadow_text = self.title_font.render("CARTE EXPLORER", True, (0, 0, 0))
        shadow_rect = shadow_text.get_rect(center=(self.width // 2 + 4, self.height // 4 - 16))
        self.screen.blit(shadow_text, shadow_rect)
        self.screen.blit(title_text, title_rect)

        # Sous-titre avec plus d'espace
        subtitle_text = self.subtitle_font.render("Explorez la carte interactive", True, (200, 200, 200))
